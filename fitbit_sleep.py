...
def fetch_sleep_logs(access_token, device_id, date_obj, db):
    ...
    if hit_rate_limit:
        ...
        for sleep_log in data['sleep']:
            ...
            sleep_session_id = db.insert_sleep_session(device_id)
...
def get_sleep_data(device, db):
    ...
    last_date = db.get_sleep_checkpoint(device['id'])
    ...
    last_sync = db.get_last_synch(device['id'])
    ...
    while current_date <= end_date:
        try:
            success, rate_limited = fetch_sleep_logs(access_token, device['id'], current_date, db)
            ...
            if e.response.status_code == 401:
                ...
                if new_access and new_refresh:
                    db.update_device_tokens(device['id'], new_access, new_refresh)
    ...
    while True:
        ...
        try:
            ...
            for device in devices:
                result = get_sleep_data(device, db)
                results[result] += 1
